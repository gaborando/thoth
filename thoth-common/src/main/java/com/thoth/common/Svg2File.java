...
public class Svg2File {

    public static final Logger logger = LogManager.getLogger(Svg2File.class);
    ...
    public static byte[] convertToJpeg(String svg, String requestId) {
        for(int i = 0; i<3; i++) {
                ...
                try (Playwright playwright = Playwright.create()) {
                    ...
                    try (Browser browser = playwright.chromium().launch()) {
                        Browser.NewContextOptions contextOptions = new Browser.NewContextOptions()
                                .setColorScheme(ColorScheme.LIGHT); // Force light mode
    ...
    public static byte[] convertToPdf(String svg, String requestId) {
        for(int i = 0; i<3; i++) {
            logger.debug("{} - Convert attempt ({})", requestId, i+1);
            try {
                try (Playwright playwright = Playwright.create()) {
                    ...
                    try (Browser browser = playwright.chromium().launch()) {
                        ...
                        try(var context = browser.newContext(contextOptions)) {
                            try (var page = context.newPage()) {
                                ...
                                // Evaluate SVG size after scaling
                                JSHandle box = page.evaluateHandle("""
                                    () => {
                                ...
                                """);
                                Map<String, Object> size = (Map<String, Object>) box.jsonValue();

                                double width = ((Number) size.get("width")).doubleValue();
                                double height = ((Number) size.get("height")).doubleValue();

                                // Convert pixels to inches (PDF dimensions are in inches), assuming 96 DPI
                                double dpi = 96.0;
                                double widthInInches = width / dpi;
                                double heightInInches = height / dpi;

                                // Set the page size explicitly
                                var pdfOptions = new Page.PdfOptions()
                                        .setWidth(widthInInches + "in")
                                        .setHeight(heightInInches + "in")
                                        .setPrintBackground(true);
                                var resp = page.pdf(pdfOptions);
                                logger.debug("{} - Playwright {} - browser {} - page  {} - Screenshot Taken!", requestId, playwright.toString(), browser, page);
                                return resp;
                            }
                        }
